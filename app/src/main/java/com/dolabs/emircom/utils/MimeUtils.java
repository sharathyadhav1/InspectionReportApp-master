...
public final class MimeUtils {
    ...
    public static String guessMimeTypeFromUri(Uri uri) {
        // try the content resolver
        String mimeType;
        try {
            mimeType =  SessionContext.mContext.getContentResolver().getType(uri);
        ...
        if (mimeType == null && uri.getPath() != null) {
            ...
            if (start < path.length()) {
                mimeType =guessMimeTypeFromExtension(path.substring(start));
            }
        }
    ...
    public static String guessExtensionTypeWithDotFromUri(Uri uri) {

        String extension = guessExtensionTypeFromUri(uri);
    ...
    public static String guessExtensionTypeFromUri(Uri uri) {

        String extension = null;

        try {

            String mimeType = guessMimeTypeFromUri(uri);
            if (mimeType != null) {
    ...
    private static String getExtensionFromUri(Uri uri) {
        ...
        Cursor cursor = SessionContext.mContext.getContentResolver().query(uri, projection, null, null, null);
